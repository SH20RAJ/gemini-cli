  ...
  getCoreSystemPrompt(
    ...
    if (isPlanMode) {
      ...
      planModeToolsList = allActiveTools
        .filter(
          (t) =>
            // Exclude write_file and edit as they are explicitly handled in renderPlanningWorkflow
            isAllowedInPlanMode(t) &&
            t.name !== WRITE_FILE_TOOL_NAME &&
            t.name !== EDIT_TOOL_NAME,
        )
        .map((t) => {
